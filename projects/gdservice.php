<?php 
$your_meta_title = "GDservice";
$your_meta_description = "Providing Bus and coach cleaning services, and experienced drivers. Designed and developed a small website focusing on SEO ranking and page speed.";
include(dirname(__FILE__) . "/../includes/page_header.php");
   
?>
    <section>
        <header class="hero-wrapper post gdservice">
            <div>
                <h1>GDservice</h1>
                <p>Providing Bus &amp; coach cleaning services, and experienced drivers. Designed and developed a small website focusing on SEO ranking and page speed. </p>
                <a class="active" href="http://www.gdservice.se" target="_blank">view website</a>
            </div>
        </header>
        <article class="project-container">
            <div class="project-context-info">
                <ul class="project-context-info-list">
                    <li>
                        <strong>Role</strong>
                        <span>Designer, Front-End Developer, SEO</span>
                    </li>
                    <li>
                        <strong>Client</strong>
                        <span>GDservice</span>
                    </li>
                    <li>
                        <strong>Date</strong>
                        <span>2016</span>
                    </li>
                </ul>
            </div>
            <div class="project-presentation">
                <picture class="design-showcase flex100">
                    <source media="(max-width: 750px)" srcset="/../images/gdservice-home-page-mobile2x.webp 2x, /../images/gdservice-home-page-mobile3x.webp 3x" type="image/webp">
                    <source  srcset="/../images/gdservice-home-page.webp" type="image/webp">

                    <source media="(max-width: 750px)" srcset="/../images/gdservice-home-page-mobile2x.jpg 2x, /../images/gdservice-home-page-mobile3x.jpg 3x">
                    <source  srcset="/../images/gdservice-home-page.jpg">
                    <img src="/../images/gdservice-home-page.jpg" alt="GDservice Home Page Web Design">
                </picture>            
            </div>
                <div class="project-presentation">
                <picture class="design-showcase">
                    <source media="(max-width: 750px)" srcset="/../images/gdservice-pagespeed-insights-mobile2x.webp 2x, /../images/gdservice-pagespeed-insights-mobile3x.webp 3x" type="image/webp">
                    <source  srcset="/../images/gdservice-pagespeed-insights.webp" type="image/webp">

                    <source media="(max-width: 750px)" srcset="/../images/gdservice-pagespeed-insights-mobile2x.jpg 2x, /../images/gdservice-pagespeed-insights-mobile3x.jpg 3x">
                    <source  srcset="/../images/gdservice-pagespeed-insights.jpg">
                    <img src="/../images/gdservice-pagespeed-insights.jpg" alt="GDservice PageSpeed Insights - Google Developers">
                </picture>  
                <div>
                <h3>PageSpeed</h3>
              <p>Striving towards a fast web experience by following Google Developers PageSpeed Insights tool.</p>
               </div>
            </div>
        </article>
        <aside class="next-project">
            <div class="next-project-container">
                <a href="#"></a>
                <h4>Next Project</h4>
                <h5 class="next-project-title"></h5>
            </div>
        </aside>
    </section>
    
<?php include(dirname(__FILE__) . "/../includes/page_footer.php");?>
    
    